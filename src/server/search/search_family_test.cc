...
TEST_F(SearchFamilyTest, AggregateSortByJson) {
  ...
  /* Test parsing errors of SORTBY */

  // Test SORTBY with invalid argument count
  resp = Run({"FT.AGGREGATE", "index", "*", "SORTBY", "999", "@name", "@number", "DESC"});
  EXPECT_THAT(resp, ErrArg("Bad arguments for SORTBY: Expected an argument, but none provided"));

  // Test SORTBY with negative argument count
  resp = Run({"FT.AGGREGATE", "index", "*", "SORTBY", "-3", "@name", "@number", "DESC"});
  EXPECT_THAT(resp, ErrArg("Bad arguments for SORTBY: Value is outside acceptable bounds"));

  // Test MAX with invalid value
  resp = Run({"FT.AGGREGATE", "index", "*", "SORTBY", "1", "@name", "MAX", "-10"});
  EXPECT_THAT(resp, ErrArg("Bad arguments for MAX: Could not convert argument to expected type"));
  ...
  EXPECT_THAT(resp,
              ErrArg("Bad arguments for SORTBY: Could not convert argument to expected type"));
}
